...
\begin{frame}{System overview}
    ...
    \begin{tikzpicture}
        ...
        \draw node[dut] (dut) {}
              ...
              node[below=of mul.center] (gamma) {$\Gamma_{set}$};
    ...
    \begin{columns}
        \column{5cm}
            ...
            \begin{tikzpicture}
                ...
                \draw [-latex,incident] ($(dirvna.A2) + (-0.2,0.1)$) -- node[base left] {$a_1$} ($(oszivna.A1 |- oszimiddle) - (0.2,0.1)$);
                \begin{pgfonlayer}{foreground}
                    \draw [-latex,reflect] ($(dirvna.B2) + (0.2,0.1)$) -- node[coordinate] (a1) {} ($(oszimiddle -| dirvna.B2) + (0.2,-0.1)$);
                \end{pgfonlayer}
                \draw (a1) node[base right,reflect] {$b_1$};
        ...
        \column{5cm}
            \begin{enumerate}
                ...
                \tikzset{external/export next=false}
                \item $\Gamma_L = \frac{\textcolor{reflect}{a_2}}{\textcolor{incident}{b_2}} \tikz[remember picture,baseline] \node[anchor=base] (approx) {$\approx$}; \frac{\textcolor{reflect}{b_1}}{\textcolor{incident}{a_1}}$
                    \tikzset{external/export next=false}
                    \tikz[overlay,remember picture] \draw [latex-,alert=2] (approx.south) -- ++(0,-2em) node[below,alert=2] {Systematic Errors};
            \end{enumerate}
    \end{columns}
\end{frame}
