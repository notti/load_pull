...
\newcommand{\clk}[1]{{\itshape\ttfamily #1\_clock}}
...
\begin{description}
    \item[\clk{sample}:] \hfill \\
        Clock recovered from LVDS (GTX). \SI{100}{MHz}.
    \item[\clk{core}:] \hfill \\
        This clock is only used for the DSP core, to be able
        to speed up the calculations. Due to synthesis constraints it is
        currently the same as \clk{sample}. \SI{100}{MHz}.
    \item[\clk{cpu}] \hfill \\
        Internal CPU bus clock. \SI{100}{Mhz}; not aligned to
        \clk{sample}.
\end{description}
Since \clk{sample} and \clk{cpu} are not related, all affected memory
...
\begin{description}
    ...
    \item[\module{GTX}:] \hfill \\
        GTX transceiver configured by wizard. This component has to be
    ...
    \item[\module{descramble}:] \hfill \\
        Descrambler modelled after \cite{ltc2274} page 28.
...
\end{description}
Since there are three GTX transceivers easily accessible on the ML507 board
...
\begin{figure}[bh]
    ...
    \begin{pspicture}(9.2,6.8)
        \rput[bl](-0.2,0){
        {
            ...
            \fnode[linecolor=red,framesize=5 1.5](4.6,5.6){fft}
            \nput{90}{fft}{\textcolor{red}{\module{fftncmul}}}
            \pspolygon[linecolor=purple](7.4,6.3)(9.2,6.3)(9.2,0.5)(3.8,0.5)(3.8,4.8)(7.4,4.8)
        ...
        & [mnode=dia] last & & \psframebox{\shortstack{$+$\module{scratch}\\$\rightarrow y[n]$}} & \\
        & [mnode=oval] finished & & &
        \psset{arrows=->}
        \ncline{1,1}{1,2}
        \ncline{1,2}{1,4}\naput{wait}\nbput{FFT}
        ...
        \ncline{2,2}{1,2}\nbput{No}
        \ncangles[armA=0.7,angleA=-90]{3,3}{4,4}
        \ncline{4,4}{4,2}\nbput{wait}\naput{iFFT}
        \ncline{4,2}{5,2}\nbput{Yes}
        \endpsmatrix}
    \end{pspicture}
