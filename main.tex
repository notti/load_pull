...
via \device{$C_5$}, used as an \gls{ac} short, and \device{$R_{10}$}, needed for input matching, to the TX+
...
switching threshold is set by the voltage divider $R_5$/$R_6$. After reaching this threshold at the input pin \device{6} of \device{IC2B}
...
convergent rounding. This rounding mode was used, to prevent bias. It works by rounding
...
and addition. This causes the needed bit widths to grow after every butterfly.
...
needs to be processed by the four steps. In the first step (\cref{fig:stage1}),
...
output. This prevents wrap around and therefore limits signal distortion.
