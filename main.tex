...
\usetikzlibrary{positioning,dsp,chains,fit,scopes,calc,backgrounds,arrows,decorations.pathmorphing,bending,arrows.meta,shapes.misc,matrix,patterns,shadows}
\include{rfsymbols}
...
an additional measurement was acquired in \cref{sec:drift}, to verify the origin
of this phase drift.
...
filter out the aliases (see \cref{sec:digital}). For the duration of the measurements,
...
\SI{900}{\mega\hertz} (see \cref{sec:matlab}). All the following
measurements used the same settings as \cref{sec:iterative}. A sample period $n$ of 20000,
...
values: $1$, $-1$, $45\angle\SI{135}{\degree}$ and $45\angle\SI{-45}{\degree}$. Additional
verification measurements were carried out with the mean of the acquired measurements
at $1$ and $1$, as well as the mean of the measurements at $45\angle\SI{135}{\degree}$
and $45\angle\SI{-45}{\degree}$. For the following verification measurements, the respective
...
of 2000 and a \gls{fft} width $n_{fft}$ of 4096 (see \cref{fig:overlap_add}). Since
according to \cref{sec:digital} \device{fir1} needs at least a low pass filter to
filter out the aliases, it was preloaded with the impulse response of a low pass filter
...
\cref{fig:phase_single}. Since the magnitude did not change noticeably during those measurements
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Phase drift of measurement system}
    \label{fig:phase_single}
\end{figure}

These results indicate, that the phase drift is not caused by the filter implementation of
\device{fir1}, which would also cause changes in the magnitude. Because of that, the test
...
the different signals generated by the signal generators. Because the frequencies
...
generate the greatest common divisor of those frequencies, which is \SI{10}{\mega\hertz}. Since
it was not possible to generate a good enough trigger signal with this function generator that
allowed reliably measuring the phase of the different signals, this signal was also recorded.
...
offset of the trigger signal of zero. This is the same as if the oscilloscopes would
have triggered perfectly. For a list of the used instruments see \cref{sec:instruments}.
Like the measurement of the whole test setup, a phase offset was acquired every \SI{10}{\second}
for \SI{1000}{\minute}.
...
The measurement results, which can be seen in \cref{fig:phase_overall} depict a
...
the accuracy needed of the measurements limit the valid time of the calibration of
