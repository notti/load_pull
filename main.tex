...
The complete source code of the kernel module can be found in \cref{sec:sources}.
...
\begin{lstlisting}[float=htb,caption={Character device initialization},label=src:cdevinit,basicstyle=\hack\tiny]
#define USER_MEM 4
...
\begin{lstlisting}[float=htb,caption={Character device access functions},label=src:cdev,basicstyle=\hack\tiny]
...
general \lstinline+show+ and \lstinline+store+ functions and the following
accompanying macros were developed:
\begin{description}
...
which is protected by a spin lock. This spin lock isn't actually needed by an
uni processor system and gets optimized away during the compilation. Since
this code could also be used on a different processor it is best practice to keep
the spin lock. The source code implementing and using this macros can be found in
...
\begin{itemize}
    \item python
    \item webinterface \rightarrow json
    \item screenshot?
