...
voltage and current do not vary over the physical dimension of the elemnts is of limited
...
can be seen.
\begin{equation}
    \label{eq:s11ref} \Gamma = S_{11} = \frac{Z_{in}-Z_0}{Z_{in}+Z_0} \cite{pozar_mw_engineering_2011}
\end{equation}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \draw [latex-] ($(iso.west) + (0.2,-0.2)$) -- node[below] {$a_2$} ($(amp.east) + (-0.2,-0.2)$);
