...
At \gls{rf} and \gls{mw} frequencies the lumped element model is of limited
value. The wavelength at these frequencies is on the order of the circuit
element dimensions. This means that there are no planes of equal electromagnetic
potential any more and the line integrals become dependent on the chosen path.
Because of this Kirchoff's laws are not applicable at these frequencies. Although
it is often possible to fix these shortcomings by adding parasitic capacitances,
it is easier to use the concept of forward and backward traveling power waves.
...
In a 1-port there is only $S_{11}$. This parameter is equivalent with the reflection
coefficient $\Gamma$ and can be also expressed by the input impedance $Z_{in}$ (see
\cref{eq:s11ref}). This does not necessarily hold for 2-ports, since for a fully
...
be connected to port 2. According to \cref{eq:s11ref} the reflected wave $b_2$
...
and reflected wave at port 1 (see \cref{eq:s11}).
\begin{align}
...
Because of this non linearities more complex models and verification systems are
...
controllable load impedance (output tuner) the \gls{dut}. The fixed input tuner is
...
coefficients. The repeatability is needed to ensure acurate application
...
resolution is an additional tradoff. High resolution is needed, because \glspl{pa} are
highly sensitive to impedance variations, but a high resolution incurs
...
the desired measurements, additional needed circuit elements like directional couplers,
...
These tuners have to be calibrated before use and achieve reflection levels like in
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \draw node[dut] (dut) {}
              ...
              node[amplifier,right=of circ,label=below:{Amplifier},yshift=-1cm,rotate=180,scale=0.6,anchor=east] (amp) {};
...
Closed loop active load-pull setups generate a modified reflected wave $a_2$ by
...
is an example of such a closed loop active load-pull setup. With the circulator
the power wave $b_2$ is fed to a variable attenuator, a phase shifter and a
amplifier. Those elements enable modifying the phase and the magnitude of $b_2$,
which is again fed back to the \gls{dut} with the circulator. Because of limited
isolation provided by real world circulators this system is able to oscillate. This
can be compensated by introducing a bandpass filter into the loop and further improved
with an isolator after the amplifier \cite{ghannouchi_load-pull_2013}.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \draw node[dut] (dut) {}
              ...
              node[amplifier,rotate=180,scale=0.6,right=of iso.west,anchor=east,label=below:Amplifier] (amp) {}
              node[vphase,right=of amp.west,label=below:{Phase Shift}] (shift) {}
...
Open loop active load-pull setups work by generating a phase coherent wave with
...
phase shifter. A disadvantage of the closed loop system is that the generated wave at
...
Therefor an iterative approach is needed to be able to achieve specific $\Gamma_L$ \cite{muller_comparison_1994}.
