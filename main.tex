...
\newglossaryentry{blocksize}%
{%
    name={\ensuremath{L}},
    description={Overlapp add block size},
    sort={L}
}
...
subsequences of length \gls{blocksize} (see \cref{fig:overlap_add}).
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
...
\begin{equation}
    \label{eq:sum_seq} x[n] = \sum_{i=0}^{\infty} x_i[n-i\gls{blocksize}] \qquad x_i[n] =\begin{cases}
        x[n+i\gls{blocksize}] & n = 0, 1, \ldots, \gls{blocksize}-1\\
...
\begin{equation}
    \label{eq:bit_growth}
    bits_{out} = bits_{in} + \underbrace{\log_2\left(n_{fft}\right)}_{\text{addition per butterfly}} + \underbrace{1}_{\text{complex rotation}}
\end{equation}
According to \cref{eq:bit_growth}, keeping every bit for an \gls{fft} length $n_{fft}$ of 4096
...
\cref{fig:overlap_add_machine}. Every block of size $L$
...
beginning (see \cref{sec:digital}). Since this process needs $L - n_{fft}$ samples
...
length $n$ the \device{outbuf} module has to be reset or a \device{resync} operation
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        { [rounded corners=2pt]
            ...
            \draw [-latex] (dircirc.A2) |- (downmix) node[above,near end] {$b_2$};
            \draw (dirvna.A2) -- (oszivna.A1);
            \draw (dirvna.B2) |- ($(dirvna.B2)!.7!(oszivna.A2)$) node[coordinate] (oszimiddle) {} -| (oszivna.A2);
            \draw [-latex] (att) -- (dircirc.B1) node[midway,above] {$a_2$};
        }
        { [-latex]
            \draw (downsource) -- (downmix);
            \draw (sample) -- (adc);
...
one port \gls{vna} is needed to be able to measure the current $\Gamma_L$ (see
...
incident wave $b_2$, converting it to \gls{if}, and digitally sampling the signal.
Furthermore it handles digital-to-analog conversion, up-conversion to \gls{rf} and
feeding back the reflected wave $a_2$ to the \gls{dut}. The digital part as described
in \cref{sec:digital} handles filtering the signal to enable broadband reflection
synthesis and applying the reflection coefficient $\Gamma_{L,set}$. Since analog and
...
the vector signal generator was turned off. Stepping through the frequency range %why turned off?
...
calibration measurements were acquired for 21 $\Delta{}f$-aligned points within a
...
$\Gamma_{L,f}$ values for every frequency point. After the measurement, the different $\Gamma_{L,f}$ were linearly
interpolated at every $\Delta{}f$ bin. Finally the \glspl{sparam} of
...
of the chosen window function, and additionally between the calibrated points, to test the %describe three measurements more cleary
performance of the interpolation. To be able to verify the $\Gamma_L$ which was measured using the
above described setup, those measurements were compared to a measurement acquired using a
commercially available \gls{vna}. %which vna? which calkit? vna settings vna datasheet!
...
\begin{table}[htb]
    ...
    \label{tab:vna}
\end{table}
%fix table (decimal dots, distinction)

No noticeable difference has been observed for the standard error and the difference to the results from the Agilent \gls{vna} for the three
different types of points over the complete \SI{25}{\mega\hertz} range. %more clearly which differences
Thus, the linear interpolation and the chosen window function seem to be appropriate for this type of
...
a given $\Gamma_L$. All the following measurements used 20000 samples as %describe target algo and why
signal period $n$ (see \cref{sec:acquisition}), a block size $L$ of 2000 and an \gls{fft} width $n_{fft}$
...
the \gls{fpga} implementation was switched to automatic mode (see \cref{chap:fpga}). The digital reflection generation introduces %describe automatic mode
a large latency on the order of about \SI{1}{\milli\second} for above mentioned settings.
Therefore a delay time of \SI{100}{\milli\second} was used for the iterative algorithm %describe why 100ms
...
Depending on the starting point $\Gamma_{L,start}$ and the target $\Gamma_{L}$
the performance of the algorithm varied. A histogram showing 108 different
observations for different $\Gamma_{L,start}$ different $\Gamma_{L,target}$ and
...
baseband range from \SI{-13}{\mega\hertz} to \SI{13}{\mega\hertz} at a \gls{rf} of %why filter calibration, how, plot!
...
$L$ of 2000 and an \gls{fft} width $n_{fft}$ of 4096 (see also \cref{fig:overlap_add}). %move settings elsewhere
...
calibration measurement was added to the filter \device{fir1} by linearly interpolating %how added
...
with a settling time of \SI{0.5}{\second} for every frequency point. To be able to verify %why 0.5s settling
...
settings for the \gls{fpga} implementation have been used ($n = 20000$, $L = 2000$, $n_{fft} = 4096$).
...
those preparations the system was set up to realize a reflection coefficient $\Gamma_L = 1$ with
...
\device{fir1}, which would also cause changes of magnitude. Because of that, the test %why?
...
the trigger source \device{trigger} and the two oscilloscopes also experience phase drift. %better explanation
...
the different parts of the measurement setup. %improve; which freq from which, ...
