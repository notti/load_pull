...
\begin{abstract}
    Load-pull is a non-linear measurement system which operates by presenting
    a specific impedance to a device under test. Active load-pull setups
    achieve this tasks by presenting a modified wave received from the device
    under test back to the device. This type of measurement system is
    needed to characterize non-linear devices like power amplifiers and can
    ...
    During the course of this thesis an active load-pull measurement system
    capable of synthesizing reflections with a constant reflection coefficient
    over a band width of \SI{20}{\mega\hertz} was developed and verified. This
    was achieved by generating the reflection in baseband. Additionally instead
    ...
    a configurable frequency response, which is needed to correct the group
    delay introduced by cabling. Furthermore, an iterative target algorithm was
    developed to be able to achieve specific reflection coefficients even with
    highly non-linear devices which can exhibit load dependent behaviour.

    The digital reflection generation was implemented in hardware on an FPGA.
    In combination with an efficient pipelined design this allowed for fast
    update rates. Additionally software running on a processor contained in
    the FPGA was developed to be able to control the digital hardware via
    Ethernet.

    Finally the implementation was tested with a commercially available vector
...
\begin{abstract}
    ...
    eine modifizierte Version das Ausgangssignals des Prüfobjekts an dieses
    ...
    Ziel dieser Diplomarbeit ist die Entwicklung und Verifizierung eines aktiven
    ...
    erzielt. Die digitale Implementierung erlaubt die Verwendung eines digitalen
    Filters mit einem frei wählbaren Frequenzgang. Diese Filter wird benötigt
    um die Gruppenlaufzeit, welche von den Kabeln im Messaufbau verursacht wird,
    auszugleichen. Zusätzlich wurde ein iterativer Algorithmus entworfen
    um gewünschte Reflexionskoeffizienten erzeugen zu können, auch wenn das
    Prüfobjekt lastabhängiges Verhalten zeigt.

    Die digitale Reflexionserzeugung ist in Hardware in einem FPGA implementiert.
    Diese effiziente pipelinebasierende Implementierung erlaubt hohe
    Aktualisierungsraten. Weiters wurde eine Kommunikationssoftware entwickelt, die
    auf dem in dem FPGA integrierten Prozessor läuft. Diese erlaubt die Fernsteuerung
    der digitalen Hardware mittels Ethernet.

    Abschliessend wurde das Messsystem mit einem kommerziell verfügbaren Vektor
    Network Analyzer anstelle des Prüfobjekts getestet.
\end{abstract}
\end{otherlanguage}
