...
provides the necessary high speed transceivers, sufficient block \gls{ram},
and dedicated digital signal processing hardware \cite{virtex5}.
In order to minimize the development time the ML507
evaluation board was used, which contains the
\gls{sata} connector needed for the \gls{adc} to be connected to a high speed GTX transceiver of the \gls{fpga}. Furthermore, high speed differential
...
\tikzset{inbuf/.style={color=Dark2-4-1}}
\tikzset{core/.style={color=Dark2-4-2}}
\tikzset{outbuf/.style={color=Dark2-4-3}}
\tikzset{pc/.style={color=Dark2-4-4}}
...
The design overview, as can be seen in \cref{fig:fpga_overview}, describes
...
The remaining embedded processor design, consisting of the \gls{cpu} and the
peripherals connected to the \gls{plb}, is described in \cref{sec:processor}.

Since this design uses the unrelated clocks from the \gls{plb} and the
\gls{adc} clock synchronization was needed to minimize
the chance of metastable processes. This was achieved using two-stage synchronizers and
...
is caused by the implementation as will be described in \cref{sec:overlap_add}, which needs an
additional buffer space of at least $L - n_{fft}$ samples after the signal (see \cref{fig:overlap_add}).
