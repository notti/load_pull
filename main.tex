...
performance of the interpolation. To be able to verify the $\gls{GammaL}$ which was measured using the
...
\begin{table}[htb]
    ...
    \begin{tabular}{rrrr}
        \toprule
        \shortstack[r]{\textbf{frequency} \\ \si{\mega\hertz}} & \shortstack[r]{887.5\\(cal)} & \shortstack[r]{887.505\\($\frac{1}{2}\Delta{}\gls{f}$)} & \shortstack[r]{888.095\\(between cal)} \\
        \midrule
...
a given $\gls{GammaL}$. All the following measurements used 20000 samples as %describe target algo and why
...
$L$ of 2000 and an \gls{fft} width $\gls{nfft}$ of 4096 (see also \cref{fig:overlap_add}). %move settings elsewhere
