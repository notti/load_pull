...
evaluation board was used for this work. This evaluation board contains the necessary
\gls{sata} connector needed for the \gls{adc}, connected to high speed GTX transceivers in the \gls{fpga}, an
Ethernet port, enabling connecting the board to a \gls{pc}, and high speed differential
pin headers, which can be used for the digital \gls{iq} interface. Although according to
...
\device{core} module. This module is described in more detail in \cref{sec:processor_interface}.
...
\gls{adc}, clock synchronization was needed, to minimize the possibility
of metastability. This was achieved with two-stage synchronizers and
...
clock muxing (eg. in \cref{sec:acquisition}). Undefined behaviour
can occur, if the timing conditions of the address signals are violated
and the enable signal is high. This can lead to memory corruption
even if write enable is not asserted\cite{virtex5}. Because of this, the
enable signal of the block \gls{ram} is de asserted during clock transitions.
Since this is not possible if the sample clock is unstable, the validity of
the sample buffers is not guaranteed, if the \gls{adc} is disconnected or
connected.

To prevent bias in the signal processing, the digital signal processing chain
used convergent rounding. This rounding mode rounds to even integers in case
of a tie. Thus the rounding does not introduce a bias towards infinity or zero.
Additionally overflows are signaled and saturation is used where applicable.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \node[draw,anchor=west] at ($(average.south west) + (0,-2em)$) (trigger) {trigger};
        \node[draw,anchor=east] at ($(average.south east) + (0,-2em)$) (wallclk) {wallclk};

        \draw[-latex] (trigger) -- (average.south -| trigger);

        \draw[-latex] (trigger) -- (wallclk);
        \draw[latex-latex] (trigger) -- (trigger -| out);
        \draw[-latex] (wallclk.south) -- ++(0,-1em) -- (\currentcoordinate -| out);

        \node[draw,fit=(average) (receiver) (wallclk),inner ysep=2em,inner xsep=0.5em,label=inbuf] (inbuf) {};
...
\begin{itemize}
    \item iqdemod
    \item xilinx fft\cite{xilinx_fft}
\end{itemize}
