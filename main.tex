...
\begin{align}
    ...
    \label{eq:X} X & = \mathcal{DFT} \left(x\,\lstinline|flattopwin($n_{sample}$, 'periodic')|\right) \\
...
leading to widely available parts for this frequency and in microwave
laboratory equipment like \glspl{vna} \cite{agilent_h70}.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[every node/.style={font=\tiny},every path/.style={decoration={name=zigzag,segment length=2pt}}]
        ...
        \begin{scope}[shift={(0,4.5)}]
            ...
            \draw (-5,0.5) node[anchor=east,font={}] {\gls{rf}};
        ...
        \begin{scope}[shift={(0,3)}]
            ...
            \bpspec{($0.7*(2,0)$)}{solid}
            \draw ($0.7*(2,0) + (0,0.1)$) -- ($0.7*(2,0) - (0,0.1)$) node[anchor=north] {$\SI{70}{\mega\hertz}$};
            \draw (-5,0.5) node[anchor=east,font={}] {\gls{if}};
        \end{scope}
        \begin{scope}[shift={(0,1.5)}]
            ...
            \draw (-5,0.5) node[anchor=east,font={}] {sampling};
        \end{scope}
        \draw [latex-latex] (-5,0) -- (5,0) node[anchor=west] {frequency};
        \draw [-latex] (0,-0.2) -- (0,1);
        \foreach \x in {-2,-1,1,2} {
        ...
        \bpspec{(0,0)}{solid}
        \draw (-5,0.5) node[anchor=east,font={}] {digital mixer};
    \end{tikzpicture}
    \caption{Down conversion frequency spectra}
