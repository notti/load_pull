...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \draw node[dut] (dut) {}
              node[dircoupler,right=2 of dut] (dirvna) {}
              node[oscilloscope,above=of dirvna.A2,anchor=A1] (oszivna) {}
              node[circulator,right=of dirvna] (circ) {}
              node[adc,right=of circ,yshift=1cm] (att) {}
              node[vphase,right=1.5 of att] (phase) {}
              node[amplifier,right=of circ,label=below:{Amplifier},yshift=-1cm,rotate=180,scale=0.6,anchor=east] (amp) {};
        ...
        \draw [rounded corners=2pt]
              (dut) -- (dirvna) -- (circ.A)
              (circ.B) -- ($(circ.B |- att)!.5!(att)$) node[coordinate] (upper) {} -- (att) -- (phase) -- ++(1,0) |- (amp) -- ($(circ.C |- amp)!.5!(amp)$) node[coordinate] (lower) {} -- (circ.C)

              (dirvna.A2) -- (oszivna.A1)
              (dirvna.B2) |- ($(dirvna.B2)!.7!(oszivna.A2)$) node[coordinate] (oszimiddle) {} -| (oszivna.A2);

        \draw [-latex] ($(dirvna.A2) + (-0.2,0.1)$) -- node[base left] {$b_{2,e}$} ($(oszivna.A1 |- oszimiddle) - (0.2,0.1)$);
        \draw [-latex] ($(dirvna.B2) + (0.2,0.1)$) -- node[base right] {$a_{2,e}$} ($(oszimiddle -| dirvna.B2) + (0.2,-0.1)$);

        \draw [dashed] ($(plane) + (0,3)$) node[anchor=south] {Load Reference Plane} -- ($(plane) - (0,2.5)$);

        \draw [latex-] ($(plane) + (0,1.5)$) -- ++(-0.5,0) node[anchor=east] {$Z_L$};

        \draw [-latex] ($(plane) + (0.25,-1)$) node[anchor=west] {$a_{2,dut}$}-- ++(-0.5,0);
        \draw [latex-] ($(plane) + (0.25,-1.5)$) node[anchor=west] {$b_{2,dut}$}-- ++(-0.5,0);
        \draw [latex-] ($(plane) + (0,-2)$) -- ++(-0.5,0) node[anchor=east] {$\Gamma_L$};
    \end{tikzpicture}
