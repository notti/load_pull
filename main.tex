...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        ...
        \draw node[dut] (dut) {}
              ...
              (dac -| inbuf) node[mixer,label=above:mul] (mul) {}
...
$\sin$ and $\cos$ waveforms with a fixed lookup table. This approach has the
...
This interface uses 8B/10B line coding over an LVDS connection \cite{jesd205B.01}. The line coding
...
channel. Clock recovery is also possible, because this protocol ensures frequent transitions
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{scope}[shift={(0,13em)}]
            \draw [decorate,decoration={brace},yshift=2pt] (0,4.1em) -- (5,4.1em) node[midway,yshift=8pt] {$n_{sample}$};
...
\end{figure}
The filter $f2$ is a causal \gls{fir} filter with length $n_{fft}$, that is
...
\begin{equation}
    \label{eq:sum_seq} x[n] = \sum_{i=0}^{\infty} x_i(n-Mi) \qquad x_i[n] =\begin{cases}
        x[n+Mi] & n = 0, 1, \ldots, L-1\\
        0 & \text{else} \end{cases}
\end{equation}
Applying the linear convolution to this sum leads to
\begin{equation}
    \begin{split}\label{eq:overlap_add_sum}
        y[n] = h[n] * x[n] & = \sum_{i=0}^{\infty} x_i(n-Mi) * h[n] = \sum_{i=0}^{\infty} y_i(n-Mi)
    \end{split}
\end{equation}
...
points are summed up by equation \cref{eq:overlap_add_sum}, this method is called overlap
...
$n_{fft} - L$ points after the sequence $y[n]$ to the beginning. The hardware implementation
of this algorithms can be seen in \cref{sec:overlap_add}.


\begin{itemize}
    \item dig iq out \cite{fsq_b17} \cite{ds90cr485}
\end{itemize}
