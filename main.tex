...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                ybar interval,
                ylabel={number of $\Gamma_L$},
                xlabel={number of iterations},
                x filter/.expression={x-1}, % remove the start point
                ymin = 0
            ]
            \addplot[blue,fill=blue!10,hist={bins=6,data min=3,data max=9}] table[y index=0] {testdata/trajectories/performance.data};
        \end{axis}
    \end{tikzpicture}
    \caption{Histogram of filter iterations needed to find a specific $\Gamma_L$}
