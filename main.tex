...
\usepackage{fontspec}
\setmonofont{Hack}[Scale=MatchUppercase]
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[circuit ee IEC,every node/.style={font=\footnotesize},circuit symbol lines/.style={draw,very thick}]
        ...
        \begin{scope}[every path/.style={decorate,decoration={brace,mirror}},every node/.style={font=\normalsize}]
            \draw ($(lpower)-(0.5cm,0.7cm)$) -- ([xshift=0.5cm]\currentcoordinate -| rpower) node[midway,below,yshift=-2pt] {signal detection};
            \draw ($(lthreshold)-(0.5cm,0.7cm)$) -- (\currentcoordinate -| rthreshold) node[midway,below,yshift=-2pt] {threshold};
            \draw ($(ldelay)-(0.5cm,0.7cm)$) -- ([xshift=0.5cm]\currentcoordinate -| out+) node[midway,below,yshift=-2pt] {time delay};
        \end{scope}
    \end{tikzpicture}
    }
...
for valid clock signals. The last part is a time delay. This time delay starts after a valid clock
...
\end{figure}
The filter \device{fir1} (see \cref{fig:digital}) is a causal \gls{fir} filter with length $n_{fft}$, the impulse response $h[n]$, and the output $y[n]$. This filter is
...
\begin{lstlisting}[float=htb,language=C,caption={Basic elements of a platform driver module},label=src:pdriver,basicstyle=\ttfamily\tiny]
...
...
\lstset{language=C,escapeinside={<*}{*>}}
\begin{lstlisting}[float=htbp,language=C,caption={Character device},label=src:cdev,basicstyle=\ttfamily\tiny]
