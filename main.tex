...
The digital part, as described in \cref{sec:digital}, was implemented on a Xilinx Virtex5
FXT. This \gls{fpga} provides the necessary high speed transceivers, enough block \gls{ram}
...
the transmitter share the internal clock generation of the GTX. Therfore the transmitter
...
\device{prepare} module transistions the clock signal back to the system clock.
...
\gls{fpga}, which is connected to HDR1\_64 on the evaluation board. After a
successful trigger the \device{trigger} module starts the data acquistion process
only after multiples of the configured signal period $n$.
...
\begin{itemize}
    \item iqdemod
    \item xilinx fft\cite{xilinx_fft} (3 block ram)
\end{itemize}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \node {TODO};
    \end{tikzpicture}
    \caption{algorithm state machine}
    \label{fig:overlap_add_machine}
\end{figure}

\begin{itemize}
    \item describe \cref{fig:overlap_add_machine}
    \item describe interface
    \item decribe specs
\end{itemize}
