...
non-linear \glspl{dut} influence the resulting \gls{GammaL} controlled by the
...
implemented. This filter enables broadband reflection synthesis with
an evenly distributed \gls{GammaL} (see \cref{sec:digital}). A description
...
\begin{enumerate}
    ...
    \item Copy the data to user space (\cref{mem_read_copy}). This must be
        done with the \mbox{\lstinline+copy_*_user+} macros. Kernel memory and
...
If user space writes a value to one of this files, then the \lstinline+store+
...
A pointer to one preallocated page is provided to the function. The function
...
\lstinline+store+ function (\mbox{\lstinline+fpga_flag_store+}) converts the textual representation
...
spin lock isn't actually needed by an uni processor system and gets optimized away during
...
(\mbox{\lstinline+fpga_flag_show+}) reads the desired value from the register, shifts it to the
...
macros that allow instantiating a single configuration option with one line of cod
...
\begin{description}
    \lstitem{FPGA_FLAG(base, name, access, register, bit, length)}
        This macro creates a flag which represents part of a register within the
...
processing setting target can be set to a specific value or queried. An example
...
directly to the \device{sysfs} files of provide by the kernel module described in
\cref{sec:kernel_module}. Therefore, the daemon just opens the file given by target
and performs the required read or write.
...
The web interface utilizing this protocol is developed in \gls{html} version 5 and JavaScript and exposes
...
web interface is served by a small web server build into the daemon. The \gls{ip}
...
\begin{itemize}
    \item matlab protocol \rightarrow text
\end{itemize}
...
\begin{itemize}
    \item short description one port vna
    \item calibration/interpolation
    \item \cref{src:manyrho}
\end{itemize}

\begin{lstlisting}[float=htb,caption={Function for calculating $\frac{\gls{a}}{\gls{b}}$ from sampled data at multiple frequencies},label=src:manyrho,basicstyle=\hack\tiny]
...
\begin{itemize}
    \item calcZ132
    \item calcErrorBoxM
    \item \cref{src:vna} script step by step
\end{itemize}

\begin{lstlisting}[float=htb,caption={One Port \gls{vna} calibration and measurement script},label=src:vna,basicstyle=\hack\tiny]
ml507 = ML507.ML507();
oszi = Scope(...);
smbv = SMBV(...);
...
[xincrement, a, b] = oszi.acquireAB(resa,resb,10e-6);
result = calcGl(S11(nf), S12(nf), S22(nf), manyRho(xincrement, a, b, f));
\end{lstlisting}
...
\begin{itemize}
    \item describe \cref{src:findtarget}
\end{itemize}

\begin{lstlisting}[float=htb,caption={Target algorithm function},label=src:findtarget,basicstyle=\hack\tiny]
function [Gl, mul] = findTarget(ml507, target, Gl_fun, p, mul)
    ...
    while true
        ...
        if abs(abs(diff) - 1) < 0.001 && abs(angle(diff)/pi*180) < 0.25
            break
        end
    end
end
...
\begin{itemize}
    \item why filter calibration
    \item describe filter calibration/interpolation
    \item \cref{src:modifyfilter}
\end{itemize}

\begin{lstlisting}[float=htb,caption={Function for applying calibration data to low-pass filter},label=src:modifyfilter,basicstyle=\hack\tiny]
...
\begin{itemize}
    \item \cref{src:filter}
\end{itemize}

\begin{lstlisting}[float=htb,caption={Filter calibration script},label=src:filter,basicstyle=\hack\tiny]
ml507 = ML507.ML507();
vna = VNA( ... );
...
nfft = 4096;
sample = 100e6;
targets = [0.5*exp(1i*pi*3/4) 0.5*exp(-1i*pi/4)];
freqs = (-13:13)*1e6;

f = designfilt('lowpassfir', 'FilterOrder', nfft-1, 'CutoffFrequency', 25e6, 'SampleRate', sample, 'Window', 'hamming');
H_orig = fft(impz(f)).';
...
H = zeros(length(targets), length(H_orig));
for i=1:length(targets)
    H = modifyFilter(H_orig, freqs, sample, nfft, multipliers(:,i));
end

%%

ml507.H = int16(H(1,:).*32767./max([max(real(H(1,:))) max(imag(H(1,:)))]));
\end{lstlisting}
...
\end{register}
\FloatBarrier %FIXME
\section{Protocol and Matlab Classes}
\begingroup
