...
\ifthenelse{\equal{\PrintVersion}{true}}{
    \tikzexternalize[mode=list and make]
    \tikzset{external/system call={lualatex \tikzexternalcheckshellescape -halt-on-error -interaction=batchmode -jobname "\image" "\texsource" && ps2pdf13 "\image".pdf "\image-13".pdf && cp "\image-13".pdf "\image".pdf}}
    \tikzexternaldisable
}{
}
...
\lstset{numbers=left,numberstyle=\tiny,stepnumber=5,numbersep=5pt,frame=single,captionpos=b,basicstyle=\ttfamily,xleftmargin=\fboxsep,xrightmargin=\fboxsep}
...
\begin{abstract}
    ...
    load-pull which retransmits a modified received wave back to the device under
    test. Generally, a load-pull system is
    intended to characterize non-linear devices like transistors and can
    also be used to test devices under different operating conditions. Traditional
    load-pull measurement systems are narrowband in nature.
    ...
    over a bandwidth of \SI{20}{\mega\hertz} was developed and verified in measurements.
    The goals were achieved by synthesizing and filtering the reflection waveform
    digitally in baseband. This filtering allowed to apply phase correction to the
    synthesized waveforms for a cyclic signal which was used to achieve a zero phase
    delay between the reflected wave and a later cycle of the received wave at the
    load reference plane. Furthermore, an iterative target algorithm based on measurements
    using an also realized 1-port vector network analyzer was developed to achieve accurate reflection coefficients.
    This algorithm needs no calibration and allows the verification of highly non-linear devices
    under test which can exhibit load dependent behaviour.
    ...
    the FPGA was developed to control the digital hardware via
    Ethernet which is needed for automated test bench setups.

    Finally, the implementation was verified by comparing the realized reflection
    coefficients to measurements carried out using a commercially available vector
...
\begin{abstract}
    Load-Pull ist eine nicht lineare Messtechnik bei der einem Prüfobjekt
    während einer Messung verschiedene Terminierungen angeboten
    werden. Aktive Load-Pull Systeme lösen diese Aufgabe mit zurück transmittieren einer
    modifizierten Version des Ausgangssignals an das Prüfobjekt. Diese Art Messung
    wird benötigt um nicht lineare Bauteile, wie z.B. Transistoren, zu charakterisieren
    und kann auch verwendet werden um Bauteile unter realen Bedingungen zu testen.
    Traditionelle Load-Pull Systeme sind von ihrer Natur aus eher Schmalbandsysteme.

    Im Zuge dieser Diplomarbeit wurde ein aktives
    Load-Pull Messsystem aufgebaut und charakterisiert, das Reflexionen über
    eine Bandbreite von \SI{20}{\mega\hertz} mit konstanten Reflexionskoeffizienten
    erzeugen kann. Dies wurde mittels digitaler Reflexionssynthese und Filterung im Basisband
    erzielt. Die Filterung ermöglicht das Ausgleichen der Gruppenlaufzeit verursacht
    durch Kabel im Messaufbau unter Nutzung eines zyklisch periodischen Eingangssignals durch
    angleichen der Phase des synthetisierten reflektierten Signals an zukünftige Signalperioden.
    Weiters wurde ein iterativer Algorithmus entworfen, der, basierend
    auf Messungen eines zusätzlich implementierten Netzwerkanalysators, den gewünschten
    Reflexionskoeffizienten erreichen kann. Dieser Algorithmus setzt keine Kalibration voraus
    und wird benötigt um Prüfobjekte mit lastabhängigem Verhalten testen zu können.

    Die digitale Reflexionsynthese ist in einem FPGA implementiert.
    ...
    der digitalen Hardware mittels Ethernet ermöglicht was in automatisierten
    Prüfaufbauten benötigt wird.

    Abschließend wurde das Messsystem verifiziert indem die erzeugten Reflexionskoeffizienten
    mit Messungen eines kommerziell erhältlichen Netzwerkanalysators verglichen wurden.
\end{abstract}
\end{otherlanguage}
...
\begin{abstract}
    ...
    through university. Especially I would like to thank Katharina for proofreading
    this thesis and for giving me the strength to withstand the sprint during
    the end of my study.

    Additional thanks go to my supervisors who always responded quickly to
...
phase of the reflected wave $\gls{a}[_2]$ to the phase of the incident wave
...
To minimize the hardware development time, a Linear Technology DC1151A-D evaluation board including the \gls{adc} was used
...
board was designed to use a \gls{sata} cable for clock and data. Furthermore, a synchronization
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \node[anchor=south west,inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.43\linewidth]{adc.jpg}};
        ...
        \end{scope}
    \end{tikzpicture}
    \caption{\glsentryshort{adc} adapter board with disconnected clock atop LTC2274 demo board}
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.48\linewidth]{digital_iq.jpg}};
    \end{tikzpicture}
...
in this design overview and the following overviews in this chapter are the actual module names used in the
source code. The overall
design was split into two parts. The first part is the digital signal processing chain
...
A reference to all hardware source codes and project files, which are needed to generate the hardware, can
...
realised with a \SI{36}{\kilo\bit} and an \SI{18}{\kilo\bit} block \gls{ram}. This leads
...
possible value. The multiplication can result in an overall bit width growth of one bit for the
...
and the \device{inactive} buffer can be initiated by asserting the \device{toggle\_buf}
...
output in case of overflows. This would prevent a wrap around and therefore limit signal distortion.
...
by starting the \device{core} module. If an overflow occurs during \device{core} module
operation, the automatic mode is stopped. After a successful filter
...
this address. This memory needs to be filled with a boot loop which
is a simple program consisting of an endless loop. Since Linux
...
components and the base system were built using buildroot\cite{buildroot}. A reference to the
...
\gls{vna} functionality were implemented as Matlab scripts. These scripts
...
\lstset{language=C,escapeinside={<*}{*>},mathescape=true,deletekeywords={register}}
...
The kernel module implemented specifically for the processor interface
...
All the code examples in this chapter are taken from the developed kernel module source. A reference to the
...
implementation of this operation. Not implemented functions need to be null pointers
...
ensure that possibly concurrent write operations to the registers don't interfere. This
...
A reference to the source code implementing and using this macros can be found in
...
interrupt number is set by the hardware implementation in \gls{xps}.
...
The \lstinline+sysfs_notify_dirent+ function wakes up user space processes
...
to prevent file system corruption. A reference to the source code for the daemon can be found in \cref{sec:sources}.
...
Matlab events. All the classes are fully documented with Matlab comments and a reference to the source codes
...
from acquired samples. A reference to the described functions and the example script \lstinline+measureVNA+
...
lag times up to \SI{1}{\milli\second}. Since the target algorithm and the digital
...
setup and cabling needed for reaching the \gls{dut}. This is achieved with
a negative phase correction for cyclic signals which works by modifying
the phase of the signal to achieve a zero phase delay between the reflected
wave and the incident wave at a later signal cycle. Furthermore,
...
A reference to an example script named \lstinline+filterCalibration+ is provided in
...
%        \draw ($(oszimiddle -| dirvna.B2) + (0.2,-0.1)$) node[base right,fill=white] {$a_1$};
        ...
        \draw [decorate,decoration={brace,mirror,amplitude=5pt}] ([yshift=-6pt]leftsplit) -- (\currentcoordinate -| fpga.south east) node[midway,below,yshift=-6pt] {Reflection Synthesis};
    \end{tikzpicture}
    }
...
synthesis part can be found in \cref{sec:reflection}. During these measurements,
...
noise of \glspl{adc} depends on the bit width and the amplitude. Therefore, small signals cause
a low \gls{snr}. Additionally, a small \gls{GammaL} value implies a small reflected wave and therefore a
lower \gls{snr}. Therefore, the resolution decreases for small $\abs{\gls{GammaL}}$. This can
        ...
        \begin{tikzpicture}[
            ...
            \def\pic#1#2{
                \begin{smithchart}[width=.9\linewidth,clip=false#1]
                    ...
                    \addplot[purple,is smithchart cs,only marks,mark size=4pt,mark=x#2] file {testdata/filter/1.0,0/trajectory10.data};
            ...
            \begin{scope}
                ...
                \pic{,every axis plot post/.append style={line width=0.04pt},every axis/.append style={line width=0.04pt,grid style={line width=0.04pt}}}{,mark size=0.4pt}
                \end{scope}
            \end{scope}
        ...
        \begin{tikzpicture}[
            ...
            \def\pic#1#2{
                \begin{smithchart}[width=0.9\linewidth,clip=false#1]
                    ...
                    \addplot[purple,is smithchart cs,only marks,mark size=4pt,mark=x#2] file {testdata/filter/0.5,-45/trajectory6.data};
            ...
            \begin{scope}
                ...
                \begin{scope}[
                    ...
                    \pic{,every axis plot post/.append style={line width=0.0057pt}}{,mark size=0.057pt}
                \end{scope}
            \end{scope}
        \end{tikzpicture}
...
in a bad resolution of the \gls{vna} used for measuring \gls{GammaL} or the digital part of the reflection synthesis in the
target range.
...
was combined with a \SI{12.5}{\mega\hertz} low-pass filter by applying the acquired $\glsdisp{GammaL}{\Gamma_{L,set}}$
to the transfer function of the filter. This calibrated filter transfer function was then
loaded in the filter implementation \device{fir1}. An example of the uncalibrated low pass
    ...
    \begin{subfigure}[b]{.45\linewidth}
        ...
        \begin{tikzpicture}
            \begin{smithchart}[width=.9\linewidth,clip=false]
                \foreach \i in {1,...,11}{
                    \foreach \j in {1,...,11}{
                        \addplot[blue,is smithchart cs,line join=bevel] file {testdata/filter/1.0,180/\i,\j.data};
                    }
                }
    ...
    \begin{subfigure}[b]{.45\linewidth}
        ...
        \begin{tikzpicture}
            \begin{smithchart}[width=.9\linewidth,clip=false]
                \foreach \i in {1,...,11}{
                    \foreach \j in {1,...,11}{
                        \addplot[blue,is smithchart cs,line join=bevel] file {testdata/filter/1.0,0/\i,\j.data};
                    }
                }
                \addplot[red,is smithchart cs,mark=*,only marks] coordinates {(+1,0)};
            \end{smithchart}
    ...
    \begin{subfigure}[b]{.45\linewidth}
        ...
        \begin{tikzpicture}
            \begin{smithchart}[width=.9\linewidth,clip=false]
                \foreach \i in {1,...,11}{
                    \foreach \j in {1,...,11}{
                        \addplot[blue,is smithchart cs,line join=bevel] file {testdata/filter/mean1/\i,\j.data};
                    }
                }
    ...
    \begin{subfigure}[b]{.45\linewidth}
        ...
        \begin{tikzpicture}
            \begin{smithchart}[width=.9\linewidth,clip=false]
                \foreach \i in {1,...,11}{
                    \foreach \j in {1,...,11}{
                        \addplot[blue,is smithchart cs,line join=bevel] file {testdata/filter/mean2/\i,\j.data};
                    }
                }
...
the different signals synthesized by the signal generators. To keep the setup as close to the previous measurements as possible, the
...
synthesize the greatest common divisor of those frequencies, which is \SI{10}{\mega\hertz}.
...
Performing the reflection synthesis digitally allowed the use of a
...
the developed hardware, web interface and protocol server, Matlab source codes, and raw verification results
...
A reference to the project files needed to build the netlist and \gls{fpga} configuration files can be found
