...
pin headers, which can be used for the digital \gls{iq} interface, are also available. Additionally, the board contains an
...
during tests it was confirmed that the necessary \SI{2}{\giga\bit\per\second}, required
by the \gls{adc} \cite{ltc2274}, are also usable. This \gls{fpga} also contains a
...
\begin{figure}[htb]
    ...
    \caption{\gls{fpga} design overview.}
...
are packed in to the \device{core}-module, in order to separate the signal processing functionality from
the rest of the \gls{fpga} design. A \device{Processor Interface} module, consisting of a memory controller,
a register implementation and a interrupt controller, enables the \gls{cpu} to interact with the
...
\gls{adc}, clock synchronization was needed. This clock synchronization to minimizes
the possibility of metastability. This was achieved with two-stage synchronizers and
additional pulse shaping, for fast changing signals, where necessary. Block
\gls{ram}, which needed to be accessed from two different clock domains, was
utilized using two ports where possible (eg. in \cref{sec:smbv_interface}), or
clock multiplexing using dedicated \gls{fpga} resources (eg. in \cref{sec:acquisition}). Undefined behaviour
can occur in the block \gls{ram}, if the timing conditions of the address signals are violated
while enable signal is high. This can lead to memory corruption
even if write enable is not asserted\cite{virtex5}. Because of this, the
enable signal of the block \gls{ram} is switched to low during clock transitions.
Since this is not possible if the sample clock is unstable, the validity of
the sample buffers is not guaranteed during port operation of the \gls{adc}.

Necessary bit width truncations throughout the digital signal processing chain use
convergent rounding. This rounding mode was used, to prevent bias. This rounding
mode rounds to even integers in case of a tie. For example rounding 4.5 with this method
...
can be also used as two independent \SI{18}{\kilo\bit} block \glspl{ram}. As
...
for the output and no address decoder is needed, because the highest address line can
be used as selection between both block \glspl{ram}. This ensures fast operation of the
overall memory. Since in one bit mode only \SI{32}{\kilo\bit} (\SI{16}{\kilo\bit})
...
additional buffer space of at least $L$ after the signal (see \cref{fig:overlap_add}).

All the hardware sources and project files, needed to generate the hardware, can
...
of the modules \device{receiver}, which handles receiving the data stream from
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption{Overview of the inbuf module.}
    \label{fig:inbuf}
\end{figure}
...
the K28.5 commas followed by either D5.6 or D16.2 code words\cite{ltc2274}. A comma is a special
code sequence of the 8b/10b line coding that does not represent valid data\cite{gtx}.
...
Because of design limitations in the ML507 evaluation
board, it is not possible to provide an externally generated clock directly to the
\gls{sata} connected GTX transceivers. Since using a clock routed through the global clock
network of the \gls{fpga} introduces a lot of jitter, an externally provided sampling clock
connected to the \gls{adc} is the preferred mode of usage \cite{gtx}.
...
K28.5. If no comma is detected during a \SI{41}{\milli\second} period, then the
...
can be bypassed therefore allowing the use of \gls{adc} that do not support
this feature.

Averaging of the data is handled by the \device{average\_mem} module. This module
consists of the above mentioned \SI{19}{\bit} by 49152 memory. Averaging is achieved
...
both ports of the block \glspl{ram}. Therefore the memory interface to the rest of the \gls{fpga}
...
\section[Overlap Add]{Overlap Add - core}
...
\gls{ram}, needed as scratch space (\device{scratch}).
...
cosine waveforms are evaluated at the point in time calculated from the
jiffy counter (see \cref{sec:acquisition}) and the current sample position.
...
\gls{fft} length at runtime, with a maximum of 4096. A further runtime configuration
setting is, that this core can change between \gls{fft} and inverse \gls{fft}. This
was allowed using only one core, therefore reducing the resource usage further. Every
computational part of the \gls{fft}, called a butterfly, consists of a multiplication
and addition. This causes the needed bit widths, to represent the samples to grow.
...
scaling schedule for the \gls{fft} and inverse \gls{fft} can be found by incrementing
the divider schedule by one until the computation stops overflowing. Input
data for the \gls{fft} core needs to be in natural order and output data is in
bit reversed order.
...
This allows speeding up the process since some parts can be done in parallel.
The first state machine controls the \gls{fft} and complex multiplication part of the
algorithm and is called \device{fftncmul}. Inverse \gls{fft} and complex
...
needs to processed by the four steps. In the first step (\cref{fig:stage1}),
...
is executed. During this step the samples are unloaded into the target memory
and the overlapping part from the previous block is added. Since the \gls{fft}
...
beginning (see \cref{fig:overlap_add}). Since this process needs the $L - n_{fft}$ samples
after the end of the signal, the sample buffer can't be used to the full extent.

The overall module is freely configurable and accepts the \gls{fft} sizes 8, 16,
...
checks in the hardware, which means that settings will lead to undefined
...
and the complex multiplication.

% ---------------------------------------------------------------------------

\section[SMBV Interface]{SMBV Interface - outbuf}
\label{sec:smbv_interface}

The \device{outbuf} module consists of the two independent sample buffers
...
signals. Both can store up to 49192 samples. A general overview can be
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption{Outbuf module overview}
    \label{fig:outbuf}
\end{figure}

Samples buffers \device{mem\_0} and \device{mem\_1} can only be accessed by the
...
that is currently streamed via the \device{transmitter} module to the digital \gls{iq}
interface. This memory can only be used in read mode via the internal interface.
The \device{active} sample buffer can be read and written to. Maintaining this
...
This module uses a configurable sample period $n$. This period can only be changed
...
module in \cref{fig:outbuf} is the multiplier that is responsible for the
reflection generation. It multiplies the signal output by to the transmitter
...
output by a configurable range of \SI{2}{\bit} to \SI{17}{\bit}. With this
shifter it is possible to reach full scale output even with small input
signals. This is for instance necessary if a high scaling factor is needed
...
output. This prevents wrap around and limits therefore signal distortion.
...
clock edges for a total of \SI{48}{\bit} per clock cycle \cite{ds90cr485}.
As specified by \cite{fsq_b17} \SI{20}{\bit} I, \SI{20}{\bit} Q data, enable,
valid, marker and trigger bits need to be encoded in these \SI{48}{\bit}. Valid
and Enable are connected to high by this implementation. The trigger and marker
bits are not supported by the Rhode \& Schwarz SMBV100a vector signal generator
in digital \gls{iq} input mode. Therefore these are permanently disabled in this
work. The \gls{fpga} internal \SI{16}{\bit} \gls{iq} implementation is mapped
...
module. After data acquisition finished, the filter operation is carried out
by starting the \device{core} module. If an overflow occurred during these
computations, then the automatic mode is stopped. After a successful filter
operation the \device{outbuf} buffer is toggled. If the \device{auto} module
was switched to continuous mode, then the whole process is repeated.

Because of the way, the \device{auto} module was implemented for this thesis,
it is necessary to clear eventual overflows of the \device{core} module.
...
Ethernet controller, a RS232 interface, a SystemACE controller,
...
\begin{figure}[htb]
    ...
    \end{tikzpicture}
    \caption{Processor design overview.}
...
a not existing address after reset. Furthermore, the \gls{cpu} starts
executing code before the complete setup is finished. To prevent undefined
...
loop is a simple program consisting only of an endless loop. Since Linux
...
but needed for debugging purposes. The SystemACE module is required to
...
the \gls{plb} interface into a \gls{xps} module allowed to keep the whole
...
memory accesses, the burst variant is needed. Wide memory accesses are needed
...
cache lines. Since the \gls{plb} is \SI{128}{\bit} wide, the non burst variant
can't handle these operations, because the module interface is only \SI{32}{\bit}
...
module. Every register has a single cycle access time.
...
processor interface. Special care was taken to enable multi cycle requests as
described above.
