...
multiplied by the complex valued $\Gamma_{set}$ (\textit{mul1}), thus creating a specific
...
an additional image will be generated by the multiplicator \textit{mul1},
...
a fully configurable filter \textit{fir1}. The setup consists of three distinct parts:
...
\textit{dir1} and an oscilloscope. The directional coupler is needed to split up
...
circulator (\textit{circ1}), which is needed for uncoupling the
...
analog to digital (\textit{bp1} and \textit{adc1}). For the sampling rate
...
filter is used as alias filter \textit{bp1}.
...
spectrum of interest in \gls{rf} around the frequency $f_0$. After the mixer \textit{mix1}, the
...
has to be used as filter \textit{bp1}. As mentioned earlier, the mixer also generates a \gls{dc} component, which is depicted as vertical dashed arrow. During sampling spectral aliases occur. These
...
filter \textit{fir1} in the next processing step. Since this filtering takes some time,
...
is the complex multiplier \textit{mul1}, which is responsible for the actual reflection
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{.45\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \end{smithchart}
        \end{tikzpicture}
        \caption{$mean(0.5 \angle 135, 0.5 \angle -45)$}
    \end{subfigure}
