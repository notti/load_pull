...
\cref{fig:test_setup}. This test setup consists of the three parts
...
$\Gamma_{L,f}$ for every frequency point. After measurement, the $\Gamma_{L,f}$ were linearly
interpolated for every $\Delta{}f$ bin. After this step the \glspl{sparam} of
the error box (see \cref{sec:vna}) were pre-calculated. 

kit \cite{zv-z132}The targets used for the calibration routine (see \cref{sec:vna}),

\begin{enumerate}
    \item errors norm distributed along frequency
    \item errors higher with less power
    \item no problem if not in bin or interpolated
\end{enumerate}
\cref{tab:vna}
\begin{itemize}
    \item what \& why
...
\begin{table}[htb]
    ...
    \begin{tabular}{rrrr}
        \toprule
        \shortstack[r]{\textbf{frequency} \\ \si{\mega\hertz}} & 887.5 & 887.505 & 888.095 \\
        \midrule
