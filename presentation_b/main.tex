    ...
    \begin{columns}
        \column{7cm}
            ...
            \begin{tikzpicture}
                ...
                { [on background layer,every path/.style={decorate,decoration={zigzag,segment length=2mm,amplitude=0.3mm}}]
                    \draw ($(dut.east)!.5!(circ.west)$) node[coordinate] (lcut) {} ++ (0,.3) -- ++(0,-0.6);
                    \draw ($(dut.east)!.6!(circ.west)$) node[coordinate] (rcut) {} ++ (0,.3) -- ++(0,-0.6);
                }
                { [rounded corners=2pt,thick]
                    \draw (dut) -- (lcut) (rcut) -- (circ.A);
...
\begin{frame}{Digital Part}
    \begin{columns}
        ...
        \column{7cm}
            ...
            \end{itemize}
            \hspace{1.5cm}\includegraphics[width=5.5cm]{digital_part}
    \end{columns}
...
\begin{frame}{Summary}
    ...
    \begin{block}{Digital Filter}
        \begin{itemize}
            ...
            \item Ensures constant $\Gamma_L$ over wide bandwidth
