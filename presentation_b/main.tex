...
\begin{frame}[t]{Digital Part}
    ...
    \begin{textblock}{7}(5.7,6.9)
        \begin{itemize}
            ...
            \item \SI{16}{\bit} signal processing chain in VHDL
        \end{itemize}
    \end{textblock}
...
\begin{frame}{Calibrated Filter, Uncalibrated $\Gamma_{L,set}$}
    ...
    \begin{columns}
        ...
        \tikzset{external/export next=false}
        \begin{tikzpicture}
            \tikzset{incident/.style={}}
            \tikzset{reflect/.style={}}
            \tikzset{generate/.style={}}
            \pgfmathsetseed{23654}
            \pgfdeclarelayer{foreground}
            \pgfsetlayers{background,main,foreground}
            \tikzpicturedependsonfile{rfsymbols.tex}
            \draw node[dut] (dut) {}
                  ...
                  (dac -| inbuf) node[mixer,label={[blue]above:mul1},generate,blue] (mul) {}
                  node[empty,right=of mul] (outbuf) {}
                  node[anchor=north] at (outbuf.south) {buffer2}

                  ($(shift)!.5!(outbuf)$) node[allpass,generate,color=red] (H) {}
                  node[anchor=west,red] at (H.east) {fir1}
                  ...
                  node[source,below=of upmix.center,scale=0.7] (upsource) {}
                  node[anchor=west] at (upsource.east) {lo4}
                  node[below=of mul.center,blue] (gamma) {$\Gamma_{L,set}$};
            ...
            { [-latex,thick]
                ...
                \draw (sample) -- (adc);
                \draw [double] (shiftsource) -- (shift);
                \draw[generate,double,blue] (gamma) -- (mul);
                \draw (upsource) -- (upmix);
            }
            { [latex-,dashed,every node/.style={font=\footnotesize},thick]
                \foreach \device in {downsource,sample,shiftsource} {
    ...
    \begin{columns}
        \column{5cm}
            \begin{tikzpicture}
                ...
                \draw[dashed] (box-1-2) -- ++(0,1.5cm) node[anchor=base] {Oscilloscope};
                \draw[dashed] (box-1-3) -- ++(0,1.5cm) node[anchor=base] {Load Reference Plane};
